...
const WINDOWINFO = struct({
	...
	ownerId: 'uint32', // custom field
	childId: 'uint32',  // custom field
});
const WinInfoPointer = ref.refType(WINDOWINFO);
...
const user32 = new ffi.Library('User32.dll', {
	...
	EnumChildWindows: ['bool', ['pointer', 'pointer', WinInfoPointer]],
});
...
const enumChildProcCallback = new ffi.Callback('bool', ['pointer', WinInfoPointer],
	function (hWnd, lParam) {
		...
		const wInfo = ref.get(lParam)
		if (wInfo.ownerId != processId) {
			wInfo.childId = processId
		}
		return true
	}
)

function getUWPAppDeepDetails (hWnd, processId) {
	const winInfo = new WINDOWINFO();
	winInfo.ownerId = processId
	winInfo.childId = winInfo.ownerId
	user32.EnumChildWindows(hWnd, enumChildProcCallback, winInfo.ref());
	
	const processHandle = kernel32.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, winInfo.childId);

	if (ref.isNull(processHandle)) {
		return { processName: undefined }; // Failed to get process handle
	}
	...
	const memoryUsage = (getProcessMemoryInfoResult !== 0) ? memoryCounters.WorkingSetSize : null

	// Close the "handle" of the process
	kernel32.CloseHandle(processHandle);

	return { processPath, processName, memoryUsage }
}
...
function windows() {
	...
	const { processPath, processName, memoryUsage } = getUWPAppDeepDetails(activeWindowHandle, processId)
